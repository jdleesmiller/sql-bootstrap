...
getBootstrapStats <- function (data, indexes) {
  t(data[indexes, .(mean(mass), var(mass))])
}
...
if (file.exists(checkFile)) {
  ...
  check <- rbindlist(mclapply(1:10000, function (trial) {
    ...
    data.table(
      trial, sampleMean,
      method = factor(rep(c('perc', 'stud', 'z', 't', '1.96'), each = 2)),
      endpoint = factor(rep(c('lo', 'hi'), times = 5)),
      value = c(bci$percent[4:5], bci$student[4:5], ciZ, ciT, ci196)
    )
