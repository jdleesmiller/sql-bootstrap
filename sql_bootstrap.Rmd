...
```{r}
trueRate <- c(0.01, 0.011)
split <- 0.5
numHits <- 1e5
abHits <- data.table(variant = ifelse(runif(numHits) < split, 1, 2))
abHits[, converted := runif(numHits) < trueRate[variant]]
xtabs(~ variant + converted, data = abHits)
```

```{r}
abResults <- abHits[, .(
  rate = mean(converted),
  se = sd(converted) / sqrt(.N),
  tLo = mean(converted) + qt(0.025, .N - 1) * sd(converted) / sqrt(.N), 
  tHi = mean(converted) + qt(0.975, .N - 1) * sd(converted) / sqrt(.N),
  n = .N
), .(variant)][order(variant)]
abResults
```

CI formula from http://www.stat.yale.edu/Courses/1997-98/101/meancomp.htm for unknown SDs:

```{r}
abResults[, .(
  d = diff(rate),
  dse = diff(rate) / sqrt(sum(se^2)),
  tLo = diff(rate) + qt(0.025, min(n)) * sqrt(sum(se^2)),
  tHi = diff(rate) + qt(0.975, min(n)) * sqrt(sum(se^2))
)]
```
